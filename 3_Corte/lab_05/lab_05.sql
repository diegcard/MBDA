...
DECLARE 
    CO_PC SYS_REFCURSOR;
BEGIN 
    CO_PC := PC_Pqrs.CO_Pqrs_Cerrados;
    DBMS_SQL.RETURN_RESULT(CO_PC);
END;
/
...
CREATE OR REPLACE PACKAGE BODY PA_ANALISTA_CLIENTES AS
    ...
    FUNCTION CO_Pqrs_Cerrados RETURN SYS_REFCURSOR IS 
    CO_PQ_CE SYS SYS_REFCURSOR;    
    BEGIN 
        CO_PQ_CE := PC_PQRS.CO_Pqrs_Cerrados;
        RETURN CO_PQ_CE;
    END;
...
GRANT SELECT,INSERT ON pqrs TO PA_ANALISTA_CLIENTES_UBER;
...
EXECUTE DBMS_SESSION.SET_ROLE('PA_CLIENTE_');
BEGIN
