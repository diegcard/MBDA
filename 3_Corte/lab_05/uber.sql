...
/* Verifica que el atributo 'correo' cumple con un patron especifico de direccion de correo electronico */
ALTER TABLE pqrsrespuestas ADD CONSTRAINT chk_pqrsrespuestas_correo CHECK (REGEXP_LIKE(correo, '.*@(gmail\.com|hotmail\.com|outlook\.com)$'));
...
BEGIN
  IF :NEW.fechaViaje <= :NEW.fechaCreacion OR :NEW.fechaViaje <= SYSDATE THEN
    RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'La fecha de viaje debe ser superior a la fecha de creacion y a la fecha actual.');
  END IF;
END;
/
...
BEGIN
    ...
    IF v_ubicacionInicial = v_ubicacionFinal THEN
        RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'La posicion inicial y final no pueden ser iguales.');
    END IF;
END;
/
...
BEGIN
    ...
    IF v_solicitudes_activas >= 1 THEN
        RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'Un cliente no puede tener dos solicitudes activas.');
    END IF;
END;
/
...
BEGIN
    IF :NEW.fechaCreacion != :OLD.fechaCreacion OR
       ...
       :NEW.ubicacionFinal != :OLD.ubicacionFinal THEN
        RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'No se pueden modificar atributos no permitidos en la tabla solicitudes.');
    END IF;
END;
/
...
BEGIN
  IF :OLD.estado = 'Pendiente' THEN
    NULL;
  ELSE
    RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'No se permite cambiar la fecha de viaje o el precio si el estado no es Pendiente.');
  END IF;
END;
/
...
BEGIN
    IF :new.fechaViaje IS NOT NULL AND :new.fechaCreacion IS NOT NULL AND :new.fechaViaje <= :new.fechaCreacion THEN
        RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'La fecha de viaje debe ser posterior a la fecha de creacion.');
    END IF;

    IF :new.fechaViaje IS NOT NULL AND :new.fechaViaje <= SYSDATE THEN
        RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'La fecha de viaje debe ser posterior a la fecha actual.');
    END IF;
END;
/
...
BEGIN
    IF :new.estado = 'Cancelada' THEN
        IF :old.estado <> 'Cancelada' THEN
            RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'No se permite cambiar el estado a "Cancelada".');
        END IF;
    END IF;
END;
/
...
BEGIN
    RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'No se pueden eliminar solicitudes.');
END;
/
...
CREATE OR REPLACE TRIGGER TR_PQRS_TICKED_BF
...
CREATE OR REPLACE TRIGGER TR_PQRS_RADICACION_BF
BEFORE INSERT ON pqrs
FOR EACH ROW
BEGIN
...
CREATE OR REPLACE TRIGGER TR_PQRS_ESTADO_BF
BEFORE INSERT ON pqrs
FOR EACH ROW
BEGIN
    :NEW.estado := 'Abierto';
...
CREATE OR REPLACE TRIGGER TR_M_PQRS_ESTADO_BF
...
BEGIN
    ...
    ELSE
        ...
        RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'Solo se puede modificar el estado del PQRS.');
    END IF;
END;
/

-- 2. Al cerrar o rechazar la PQRS se debe asignar la fecha actual en el campo fecha de cierre.
CREATE OR REPLACE TRIGGER TR_M_PQRS_ESTADO_FECHA_BF
BEFORE UPDATE ON pqrs
FOR EACH ROW
BEGIN
...
CREATE OR REPLACE TRIGGER TR_M_PQRS_FECHA_RADICACION_BF
BEFORE UPDATE ON pqrs
FOR EACH ROW
BEGIN
    IF :OLD.cierre IS NOT NULL AND :NEW.cierre IS NOT NULL THEN
        IF :NEW.cierre <= :OLD.radicacion THEN
            RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'La fecha de cierre debe ser posterior a la fecha de radicacion.');
        END IF;
    END IF;
END;
/
...
CREATE OR REPLACE TRIGGER TR_M_PQRS_ANEXOS_BF
...
BEGIN
    ...
    BEGIN
        ...
        IF v_estado_pqrs <> 'Abierto' THEN
            RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'Solo es posible adicionar anexos a un PQRS si su estado es "Abierto".');
        END IF;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'El PQRS al que intenta adicionar un anexo no existe.');
    END;
END;
/
...
BEGIN
    ...
    IF v_respuestas_asociadas > 0 THEN
        RAISE_APPLICATION_ERROR(-code_rise_exception.NEXTVAL, 'No se puede eliminar el PQRS porque tiene respuestas asociadas.');
    END IF;
END;
/

-- 2. Al eliminar un PQRS se deben borrar los documentos anexos asociados.
CREATE OR REPLACE TRIGGER TR_PQRS_ELIMINAR_ANEXOS_BF
BEFORE DELETE ON pqrs
FOR EACH ROW
BEGIN
    DELETE FROM anexos
